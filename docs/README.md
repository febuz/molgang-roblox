...
| [P2P-THREAT-MODEL.md](P2P-THREAT-MODEL.md) §2 (sybil gate) | `src/integrations/lightrag/group-voting.ts` (DID-membership-scoped proposals), `group-events.ts` (MQTT+Kafka fan-out), `fact-matrix.ts` (888 888 888-dim sparse fact space: transactions price/volume, news, votes, backlog), `src/integrations/mqtt/mqtt-client.ts` (zero-dep MQTT 3.1.1); tests in `tests/unit/groupVoting.test.ts`, `factMatrix.test.ts`, `mqttClient.test.ts` |
