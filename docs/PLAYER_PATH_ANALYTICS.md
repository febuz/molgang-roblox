...
The existing `Analytics_v1` OrderedDataStore remains the lightweight session-duration index; path records are kept in the normal DataStore because they are structured tables. Session writes retry three times and flush through `BindToClose`, so short sessions and normal server shutdowns do not silently lose the route.
