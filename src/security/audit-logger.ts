...
export class AuditLogger {
   ...
   */
  private analyzeActivity(event: AuditEvent): void {
    ...
    if (event.status === 'failure') {
      ...
      if (pData.count > 5) {
        this.createAlert(
          'warning',
          'brute-force-attempt',
          `Multiple failed ${event.action} attempts from user ${event.userId}`,
          event.userId
        );
      }
    }
    ...
    if (hour < 6 || hour > 23) {
      this.createAlert(
        'info',
        'unusual-access-time',
        `${event.userId} accessed ${event.resource} at unusual time`,
        event.userId
      );
    }
  }
   ...
   */
  private createAlert(level: string, type: string, description: string, source: string): SecurityAlert {
   ...
   */
  getUserAuditLog(userId: string, limit: number = 100): AuditEvent[] {
   ...
   */
  getResourceAuditLog(resource: string, limit: number = 100): AuditEvent[] {
    return this.auditLog
   ...
   * Get security alerts
   */
  getSecurityAlerts(level?: string, limit: number = 100): SecurityAlert[] {
   ...
   */
  getComplianceReport(days: number = 30): any {
   ...
   * Get top actions
   */
  private getTopActions(events: AuditEvent[]): Record<string, number> {
    ...
    return Object.fromEntries(
      Object.entries(actions).sort((a, b) => b[1] - a[1]).slice(0, 10)
    );
  }
   ...
   * Get failed attempts
   */
  private getFailedAttempts(events: AuditEvent[]): Record<string, number> {
    ...
    return Object.fromEntries(
      Object.entries(failures).sort((a, b) => b[1] - a[1]).slice(0, 10)
    );
  }
   ...
   * Export audit log
   */
  exportAuditLog(format: 'json' | 'csv', limit: number = 10000): string {
  ...
  getSecurityHealthScore(): any {
    ...
    const alertCount = this.securityAlerts.filter(
      a => a.timestamp >= new Date(Date.now() - 24 * 60 * 60 * 1000)
    ).length;

    let score = 100;
    if (failureRate > 5) score -= 10;
