...
exports.NumeraiDataFetcher = void 0;
const logger_1 = __importDefault(require("../../utils/logger"));
class NumeraiDataFetcher {
    ...
    initializeDataSources() {
        ...
        logger_1.default.info(`✓ Initialized ${this.dataSources.length} data sources`);
    }
    /**
    ...
    loadEligibleAssets() {
        ...
        logger_1.default.info(`✓ Loaded ${this.eligibleAssets.length} eligible assets`);
    }
    /**
     * Fetch all data for the day
     */
    async fetchDailyData() {
        logger_1.default.info('🔄 Starting daily data fetch...');
        ...
        try {
            ...
            for (const source of this.dataSources) {
                try {
                    logger_1.default.debug(`Fetching from ${source.name}...`);
            ...
            logger_1.default.info(`✓ Daily data fetch complete: ${securitiesUpdated} securities, ` +
                `${signalsUpdated} signals, ${competitionsUpdated} competitions (${duration}ms)`);
            return result;
        }
        catch (error) {
            logger_1.default.error('Daily data fetch failed:', error.message);
            throw error;
        }
    }
    /**
    ...
    async fetchEligibleShares() {
        ...
        for (const ticker of this.eligibleAssets) {
            try {
                ...
                logger_1.default.warn(`Failed to fetch share ${ticker}:`, error.message);
            }
        }
        logger_1.default.debug(`✓ Updated ${updated} eligible shares`);
        return updated;
    }
    /**
     * Update signals for all securities
     */
    async updateSignalsForSecurities() {
        let updated = 0;
        ...
        for (const sec of securities) {
            try {
                const signal = {
                    ...
                    type: 'signal',
                    security_id: sec.id,
                    signal_name: `Primary Signal: ${sec.ticker}`,
                    description: `Numerai prediction signal for ${sec.ticker}`,
                    lookback_period: 20,
                ...
                logger_1.default.warn(`Failed to create signal for ${sec.id}:`, error.message);
            }
        }
        logger_1.default.debug(`✓ Updated ${updated} signals`);
        return updated;
    }
    /**
     * Fetch active competitions
    ...
    async fetchActiveCompetitions() {
        ...
        try {
            ...
            updated++;
            logger_1.default.debug(`✓ Updated ${updated} active competitions`);
        }
        catch (error) {
            logger_1.default.warn('Failed to fetch competitions:', error.message);
        }
        return updated;
    }
    /**
...
exports.NumeraiDataFetcher = NumeraiDataFetcher;
exports.default = NumeraiDataFetcher;
