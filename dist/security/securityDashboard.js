...
class SecurityDashboard {
    constructor(authSystem, auditLogger) {
    ...
    snapshot() {
        ...
        const events = this.auditLogger.getAllEvents(10000);
        const recent = events.filter(e => new Date(e.timestamp) >= sinceDay);
        const failedLogins24h = recent.filter(e => e.eventType === 'login' && e.outcome === 'failure');
        ...
        const suspiciousIps = Object.entries(failedByIp).filter(([, n]) => n >= SUSPICIOUS_IP_FAIL_THRESHOLD);
        ...
        if (suspiciousIps.length > 0)
            score -= Math.min(20, suspiciousIps.length * 5);
        if (criticalEvents24h.length > 0)
            score -= Math.min(15, criticalEvents24h.length * 3);
...
function countByIp(events) {
    const out = {};
    for (const e of events)
        out[e.ipAddress] = (out[e.ipAddress] || 0) + 1;
    return out;
}
