...
function registerSkills(lightrag) {
    ...
    skills.set('find_precedent', {
        ...
        execute: async (params) => {
            const precedents = await lightrag.findSimilar(params.topic, params.similarity_threshold || 0.75);
            return {
                success: true,
                precedents,
    ...
    skills.set('get_context', {
        ...
        execute: async (params) => {
            const context = await lightrag.getContext(params.project_id, params.include || []);
            return {
                success: true,
                context,
    ...
    logger_1.default.info(`✓ Registered ${skills.size} LightRAG skills`);
}
...
function executeSkill(name, params) {
