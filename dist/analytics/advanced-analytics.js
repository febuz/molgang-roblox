...
class AdvancedAnalytics {
     ...
     * Track event
     */
    trackEvent(type, agent, duration, status, metadata) {
        const event = {
            id: `event_${Date.now()}`,
            type,
            agent,
     ...
     */
    getPerformanceReport(agentName, hoursBack = 24) {
     ...
     */
    getHealthScore() {
        const recentEvents = this.events.filter(e => e.timestamp >= new Date(Date.now() - 60 * 60 * 1000));
        ...
        if (successRate < 90)
            healthScore -= 10;
        if (this.insights.some(i => i.priority === 'critical'))
            healthScore -= 20;
        return {
